...
export class MockAuthDriver extends BaseAuthDriver {
  ...
  constructor(config: MockAuthConfig) {
    ...
    // ALLOW_MOCK_IN_PRODUCTION=true bypasses this guard for local production testing only
    if (process.env.NODE_ENV === 'production' && process.env.ALLOW_MOCK_IN_PRODUCTION !== 'true') {
      throw new Error(
        'MockAuthDriver cannot be used in production. Set AUTH_DRIVER to "saml" or "entra-id", or set ALLOW_MOCK_IN_PRODUCTION=true for local testing.'
      )
    }
